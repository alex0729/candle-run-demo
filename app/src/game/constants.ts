...
// 게임 사이클: 매일 새벽 7시(개장) → 다음날 새벽 6시(마감), 6~7시는 결산(입상 발표) 구간.
// 사이클 키 = 그 사이클이 "개장한 날짜"(7시 이전이면 어제 개장분에 속함).
...
export const msToCycleClose = (d: Date = new Date()) => msToNextHour(CYCLE_CLOSE_HOUR, d) // 마감(새벽 6시)까지
export const msToCycleOpen = (d: Date = new Date()) => msToNextHour(CYCLE_OPEN_HOUR, d)   // 개장(새벽 7시)까지
