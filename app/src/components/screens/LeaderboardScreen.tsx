...
import { dayKey, monthKey } from '../../game/constants'
...
export default function LeaderboardScreen() {
  ...
  const list: RankEntry[] = daily ? getDailyRanking(dayKey()) : getMonthlyVisitRanking(monthKey())
  const myScore = daily ? s.dailyScore : s.monthlyVisits
  const played = daily ? (s.dailyDoneCount > 0) : (s.monthlyVisits > 0)
  const fmtScore = (n: number) => daily ? `${n >= 0 ? '' : '-'}₩${fmt(Math.abs(n))}` : `${fmt(n)}회`
  const subLabel = daily ? '오늘 획득 페이북겜머니' : '이번달 방문'
    ...
    <div className="screen">
      <div className="scroll">
        <div className="lb-head">
          ...
          <div className="lb-sub"><MidnightTimer /></div>
        </div>

        <div className="lb-toggle">
          <button className={cls(period === 'daily' && 'on')} onClick={() => setPeriod('daily')}>일간 (점수)</button>
        ...
        {played && me ? (
          ...
          <div className="lb-me ghost">
            <div className="mid"><div className="n">{daily ? '오늘 한 판이면 랭킹 등록' : '방문할수록 순위가 올라가요'}</div><div className="s">{daily ? '오늘의 종목을 플레이해 보세요' : '매일 접속으로 방문 횟수 UP'}</div></div>
        ...
        <div className="disclaimer" style={{ marginTop: 14 }}>랭킹은 프로토타입 예시 데이터입니다 · 일간=오늘 점수, 월간=방문횟수 기준</div>
      </div>
